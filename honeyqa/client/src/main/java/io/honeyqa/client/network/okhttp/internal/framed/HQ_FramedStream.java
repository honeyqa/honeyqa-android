...
import io.honeyqa.client.network.okio.AsyncTimeout;
...
public final class HQ_FramedStream {
  // HQ_Internal state is guarded by this. No long-running or potentially
  ...
  private final HQ_FramedConnection connection;

  /** HQ_Headers sent by the stream initiator. Immutable and non null. */
  private final List<HQ_Header> requestHeaders;

  /** HQ_Headers sent in the stream reply. Null if reply is either not sent or not sent yet. */
  private List<HQ_Header> responseHeaders;
   ...
   */
  private HQ_ErrorCode errorCode = null;

  HQ_FramedStream(int id, HQ_FramedConnection connection, boolean outFinished, boolean inFinished,
                  List<HQ_Header> requestHeaders) {
  ...
  public HQ_FramedConnection getConnection() {
    return connection;
  }

  public List<HQ_Header> getRequestHeaders() {
   ...
   */
  public synchronized List<HQ_Header> getResponseHeaders() throws IOException {
    readTimeout.enter();
    try {
   ...
   */
  public synchronized HQ_ErrorCode getErrorCode() {
    return errorCode;
  }
   ...
   */
  public void reply(List<HQ_Header> responseHeaders, boolean out) throws IOException {
    assert (!Thread.holdsLock(HQ_FramedStream.this));
   ...
   */
  public void close(HQ_ErrorCode rstStatusCode) throws IOException {
   ...
   */
  public void closeLater(HQ_ErrorCode errorCode) {
  ...
  private boolean closeInternal(HQ_ErrorCode errorCode) {
  ...
  void receiveHeaders(List<HQ_Header> headers, HQ_HeadersMode headersMode) {
    assert (!Thread.holdsLock(HQ_FramedStream.this));
    HQ_ErrorCode errorCode = null;
    boolean open = true;
    synchronized (this) {
      if (responseHeaders == null) {
        if (headersMode.failIfHeadersAbsent()) {
          errorCode = HQ_ErrorCode.PROTOCOL_ERROR;
        } else {
        ...
        if (headersMode.failIfHeadersPresent()) {
          errorCode = HQ_ErrorCode.STREAM_IN_USE;
        } else {
          List<HQ_Header> newHeaders = new ArrayList<>();
  ...
  synchronized void receiveRstStream(HQ_ErrorCode errorCode) {
    if (this.errorCode == null) {
      this.errorCode = errorCode;
      notifyAll();
    }
  ...
  private final class FramedDataSource implements Source {
    ...
    /** Buffer with readable data. Guarded by HQ_FramedStream.this. */
    ...
    @Override public long read(Buffer sink, long byteCount)
      ...
      long read;
      synchronized (HQ_FramedStream.this) {
        waitUntilReadable();
        checkNotClosed();
    ...
    void receive(BufferedSource in, long byteCount) throws IOException {
      ...
      while (byteCount > 0) {
        ...
        if (flowControlError) {
          in.skip(byteCount);
          closeLater(HQ_ErrorCode.FLOW_CONTROL_ERROR);
          return;
        }
        ...
        synchronized (HQ_FramedStream.this) {
          ...
          if (wasEmpty) {
            HQ_FramedStream.this.notifyAll();
          }
        }
      }
    }
  ...
  private void cancelStreamIfNecessary() throws IOException {
    ...
    if (cancel) {
      ...
      HQ_FramedStream.this.close(HQ_ErrorCode.CANCEL);
    } else if (!open) {
      connection.removeStream(id);
    }
  }
  ...
  void addBytesToWriteWindow(long delta) {
    ...
    if (delta > 0) HQ_FramedStream.this.notifyAll();
  }
  ...
  class StreamTimeout extends AsyncTimeout {
    @Override protected void timedOut() {
      closeLater(HQ_ErrorCode.CANCEL);
    }

 ...
 */
public final class HQ_MultipartBuilder {
  /**
   ...
   */
  public static final HQ_MediaType MIXED = HQ_MediaType.parse("multipart/mixed");
   ...
   */
  public static final HQ_MediaType ALTERNATIVE = HQ_MediaType.parse("multipart/alternative");
   ...
   * "message/rfc822".
   */
  public static final HQ_MediaType DIGEST = HQ_MediaType.parse("multipart/digest");
   ...
   */
  public static final HQ_MediaType PARALLEL = HQ_MediaType.parse("multipart/parallel");
   ...
   */
  public static final HQ_MediaType FORM = HQ_MediaType.parse("multipart/form-data");
  ...
  private HQ_MediaType type = MIXED;

  // Parallel lists of nullable headers and non-null bodies.
  private final List<HQ_Headers> partHeaders = new ArrayList<>();
  private final List<HQ_RequestBody> partBodies = new ArrayList<>();

  /** Creates a new multipart builder that uses a random boundary token. */
  public HQ_MultipartBuilder() {
   ...
   * attacks.
   */
  public HQ_MultipartBuilder(String boundary) {
   ...
   * {@link #FORM}.
   */
  public HQ_MultipartBuilder type(HQ_MediaType type) {
    if (type == null) {
  ...
  public HQ_MultipartBuilder addPart(HQ_RequestBody body) {
    return addPart(null, body);
  }

  /** Add a part to the body. */
  public HQ_MultipartBuilder addPart(HQ_Headers headers, HQ_RequestBody body) {
    if (body == null) {
  ...
  public HQ_MultipartBuilder addFormDataPart(String name, String value) {
    return addFormDataPart(name, null, HQ_RequestBody.create(null, value));
  }

  /** Add a form data part to the body. */
  public HQ_MultipartBuilder addFormDataPart(String name, String filename, HQ_RequestBody value) {
    if (name == null) {
    ...
    return addPart(HQ_Headers.of("Content-Disposition", disposition.toString()), value);
  }
  ...
  private static final class MultipartRequestBody extends HQ_RequestBody {
    private final ByteString boundary;
    private final HQ_MediaType contentType;
    private final List<HQ_Headers> partHeaders;
    private final List<HQ_RequestBody> partBodies;
    private long contentLength = -1L;

    public MultipartRequestBody(HQ_MediaType type, ByteString boundary, List<HQ_Headers> partHeaders,
        List<HQ_RequestBody> partBodies) {
      if (type == null) throw new NullPointerException("type == null");

      this.boundary = boundary;
      this.contentType = HQ_MediaType.parse(type + "; boundary=" + boundary.utf8());
      this.partHeaders = HQ_Util.immutableList(partHeaders);
      this.partBodies = HQ_Util.immutableList(partBodies);
    }
    ...
    private long writeOrCountBytes(BufferedSink sink, boolean countBytes) throws IOException {
      ...
      for (int p = 0, partCount = partHeaders.size(); p < partCount; p++) {
        HQ_Headers headers = partHeaders.get(p);
        HQ_RequestBody body = partBodies.get(p);

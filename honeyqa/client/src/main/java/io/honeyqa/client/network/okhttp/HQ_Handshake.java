 ...
 */
public final class HQ_Handshake {
  private final String cipherSuite;
  private final List<Certificate> peerCertificates;
  private final List<Certificate> localCertificates;

  private HQ_Handshake(
  ...
  public static HQ_Handshake get(SSLSession session) {
    ...
    List<Certificate> peerCertificatesList = peerCertificates != null
        ? HQ_Util.immutableList(peerCertificates)
        : Collections.<Certificate>emptyList();

    Certificate[] localCertificates = session.getLocalCertificates();
    List<Certificate> localCertificatesList = localCertificates != null
        ? HQ_Util.immutableList(localCertificates)
        : Collections.<Certificate>emptyList();

    return new HQ_Handshake(cipherSuite, peerCertificatesList, localCertificatesList);
  }

  public static HQ_Handshake get(
      String cipherSuite, List<Certificate> peerCertificates, List<Certificate> localCertificates) {
    if (cipherSuite == null) throw new IllegalArgumentException("cipherSuite == null");
    return new HQ_Handshake(cipherSuite, HQ_Util.immutableList(peerCertificates),
        HQ_Util.immutableList(localCertificates));
  }
  ...
  @Override public boolean equals(Object other) {
    if (!(other instanceof HQ_Handshake)) return false;
    HQ_Handshake that = (HQ_Handshake) other;

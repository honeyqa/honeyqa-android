...
public enum HQ_ErrorCode {
  ...
  private HQ_ErrorCode(int httpCode, int spdyRstCode, int spdyGoAwayCode) {
    this.httpCode = httpCode;
    this.spdyRstCode = spdyRstCode;
    this.spdyGoAwayCode = spdyGoAwayCode;
  }

  public static HQ_ErrorCode fromSpdy3Rst(int code) {
    for (HQ_ErrorCode errorCode : HQ_ErrorCode.values()) {
      if (errorCode.spdyRstCode == code) return errorCode;
    }
    return null;
  }

  public static HQ_ErrorCode fromHttp2(int code) {
    for (HQ_ErrorCode errorCode : HQ_ErrorCode.values()) {
  ...
  public static HQ_ErrorCode fromSpdyGoAway(int code) {
    for (HQ_ErrorCode errorCode : HQ_ErrorCode.values()) {

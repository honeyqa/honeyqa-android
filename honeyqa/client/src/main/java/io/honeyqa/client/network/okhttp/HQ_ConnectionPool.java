 ...
 * requests that share the same {@link HQ_Address} may share a
 * {@link HQ_Connection}. This class implements the policy of
 ...
 * initialized lazily.
 */
public final class HQ_ConnectionPool {
  private static final long DEFAULT_KEEP_ALIVE_DURATION_MS = 5 * 60 * 1000; // 5 min

  private static final HQ_ConnectionPool systemDefault;
  ...
  static {
    ...
    if (keepAlive != null && !Boolean.parseBoolean(keepAlive)) {
      systemDefault = new HQ_ConnectionPool(0, keepAliveDurationMs);
    } else if (maxIdleConnections != null) {
      systemDefault = new HQ_ConnectionPool(Integer.parseInt(maxIdleConnections), keepAliveDurationMs);
    } else {
      systemDefault = new HQ_ConnectionPool(5, keepAliveDurationMs);
    }
  }
  ...
  private final LinkedList<HQ_Connection> connections = new LinkedList<>();
  ...
  private Executor executor = new ThreadPoolExecutor(
      ...
      new LinkedBlockingQueue<Runnable>(), HQ_Util.threadFactory("OkHttp HQ_ConnectionPool", true));
  ...
  public HQ_ConnectionPool(int maxIdleConnections, long keepAliveDurationMs) {
    this.maxIdleConnections = maxIdleConnections;
    this.keepAliveDurationNs = keepAliveDurationMs * 1000 * 1000;
  }

  public static HQ_ConnectionPool getDefault() {
  ...
  public synchronized int getMultiplexedConnectionCount() {
    int total = 0;
    for (HQ_Connection connection : connections) {
  ...
  public synchronized HQ_Connection get(HQ_Address address) {
    HQ_Connection foundConnection = null;
    for (ListIterator<HQ_Connection> i = connections.listIterator(connections.size());
        i.hasPrevious(); ) {
      HQ_Connection connection = i.previous();
      ...
      if (!connection.isFramed()) {
        try {
          HQ_Platform.get().tagSocket(connection.getSocket());
        } catch (SocketException e) {
          HQ_Util.closeQuietly(connection.getSocket());
          // When unable to tag, skip recycling and close
          HQ_Platform.get().logW("Unable to tagSocket(): " + e);
          continue;
        }
      }
   ...
   */
  void recycle(HQ_Connection connection) {
    if (connection.isFramed()) {
      return;
    }
    ...
    try {
      HQ_Platform.get().untagSocket(connection.getSocket());
    } catch (SocketException e) {
      // When unable to remove tagging, skip recycling and close.
      HQ_Platform.get().logW("Unable to untagSocket(): " + e);
      HQ_Util.closeQuietly(connection.getSocket());
      return;
    }
  ...
  private void addConnection(HQ_Connection connection) {
   ...
   */
  void share(HQ_Connection connection) {
  ...
  public void evictAll() {
    ...
    for (int i = 0, size = toEvict.size(); i < size; i++) {
      HQ_Util.closeQuietly(toEvict.get(i).getSocket());
    }
  }
  ...
  boolean performCleanup() {
    List<HQ_Connection> evictableConnections;
    ...
    for (int i = 0, size = evictableConnections.size(); i < size; i++) {
      HQ_Connection expiredConnection = evictableConnections.get(i);
      HQ_Util.closeQuietly(expiredConnection.getSocket());
    }
   ...
   */
  // VisibleForTesting
  synchronized List<HQ_Connection> getConnections() {

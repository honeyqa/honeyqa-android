...
import static io.honeyqa.client.network.okio.ByteString.EMPTY;

/**
 * Read and write HTTP/2 frames.
 * <p>
 * This implementation assumes we do not send an increased
 * {@link HQ_Settings#getMaxFrameSize frame size setting} to the peer. Hence, we
 ...
 */
public final class HQ_Http2 implements HQ_Variant {
  private static final Logger logger = Logger.getLogger(FrameLogger.class.getName());

  @Override public HQ_Protocol getProtocol() {
    return HQ_Protocol.HTTP_2;
  }
  ...
  static final class Reader implements HQ_FrameReader {
    ...
    // Visible for testing.
    final HQ_Hpack.Reader hpackReader;

    Reader(BufferedSource source, int headerTableSize, boolean client) {
      this.source = source;
      this.client = client;
      this.continuation = new ContinuationSource(this.source);
      this.hpackReader = new HQ_Hpack.Reader(headerTableSize, continuation);
    }
    ...
    private void readHeaders(Handler handler, int length, byte flags, int streamId)
      ...
      List<HQ_Header> headerBlock = readHeaderBlock(length, padding, flags, streamId);

      handler.headers(false, endStream, streamId, -1, headerBlock, HQ_HeadersMode.HTTP_20_HEADERS);
    }

    private List<HQ_Header> readHeaderBlock(int length, short padding, byte flags, int streamId)
        throws IOException {
    ...
    private void readRstStream(Handler handler, int length, byte flags, int streamId)
      ...
      HQ_ErrorCode errorCode = HQ_ErrorCode.fromHttp2(errorCodeInt);
      if (errorCode == null) {
  ...
  static final class Writer implements HQ_FrameWriter {
    ...
    private final HQ_Hpack.Writer hpackWriter;
    ...
    Writer(BufferedSink sink, boolean client) {
      ...
      this.hpackWriter = new HQ_Hpack.Writer(hpackBuffer);
    ...
    @Override public synchronized void ackSettings(HQ_Settings peerSettings) throws IOException {
      if (closed) throw new IOException("closed");
    ...
    @Override public synchronized void pushPromise(int streamId, int promisedStreamId,
        List<HQ_Header> requestHeaders) throws IOException {
      if (closed) throw new IOException("closed");
    ...
    void headers(boolean outFinished, int streamId, List<HQ_Header> headerBlock) throws IOException {
      if (closed) throw new IOException("closed");

...
import io.honeyqa.client.network.okhttp.internal.HQ_ConnectionSpecSelector;
import io.honeyqa.client.network.okhttp.internal.HQ_Platform;
...
import io.honeyqa.client.network.okhttp.internal.http.HQ_FramedTransport;
import io.honeyqa.client.network.okhttp.internal.http.HQ_HttpConnection;
import io.honeyqa.client.network.okhttp.internal.http.HQ_HttpEngine;
import io.honeyqa.client.network.okhttp.internal.http.HQ_HttpTransport;
 ...
 * HTTP connections as members of a {@linkplain HQ_ConnectionPool connection pool}.
 *
 ...
 *   <li>Application Layer HQ_Protocol Negotiation (ALPN) enables the HTTPS port
 ...
 */
public final class HQ_Connection {
  private final HQ_ConnectionPool pool;
  private final HQ_Route route;

  private Socket socket;
  private boolean connected = false;
  private HQ_HttpConnection httpConnection;
  private HQ_FramedConnection framedConnection;
  private HQ_Protocol protocol = HQ_Protocol.HTTP_1_1;
  ...
  public HQ_Connection(HQ_ConnectionPool pool, HQ_Route route) {
    this.pool = pool;
    this.route = route;
  }
  ...
  void setOwner(Object owner) {
    ...
    synchronized (pool) {
      if (this.owner != null) throw new IllegalStateException("HQ_Connection already has an owner!");
      this.owner = owner;
    }
  }
  ...
  void connect(int connectTimeout, int readTimeout, int writeTimeout, HQ_Request request,
      List<HQ_ConnectionSpec> connectionSpecs, boolean connectionRetryEnabled) throws HQ_RouteException {
    if (connected) throw new IllegalStateException("already connected");

    HQ_RouteException routeException = null;
    HQ_ConnectionSpecSelector connectionSpecSelector = new HQ_ConnectionSpecSelector(connectionSpecs);
    Proxy proxy = route.getProxy();
    HQ_Address address = route.getAddress();

    if (route.address.getSslSocketFactory() == null
        && !connectionSpecs.contains(HQ_ConnectionSpec.CLEARTEXT)) {
      throw new HQ_RouteException(new UnknownServiceException(
    ...
    while (!connected) {
      try {
        ...
        HQ_Util.closeQuietly(socket);
        socket = null;

        if (routeException == null) {
          routeException = new HQ_RouteException(e);
        } else {
  ...
  private void connectSocket(int connectTimeout, int readTimeout, int writeTimeout,
      HQ_Request request, HQ_ConnectionSpecSelector connectionSpecSelector) throws IOException {
    socket.setSoTimeout(readTimeout);
    HQ_Platform.get().connectSocket(socket, route.getSocketAddress(), connectTimeout);

    if (route.address.getSslSocketFactory() != null) {
      connectTls(readTimeout, writeTimeout, request, connectionSpecSelector);
    }

    if (protocol == HQ_Protocol.SPDY_3 || protocol == HQ_Protocol.HTTP_2) {
      socket.setSoTimeout(0); // Framed connection timeouts are set per-stream.
      framedConnection = new HQ_FramedConnection.Builder(route.address.uriHost, true, socket)
          .protocol(protocol).build();
      framedConnection.sendConnectionPreface();
    } else {
      httpConnection = new HQ_HttpConnection(pool, this, socket);
    }
  }

  private void connectTls(int readTimeout, int writeTimeout, HQ_Request request,
      HQ_ConnectionSpecSelector connectionSpecSelector) throws IOException {
    ...
    try {
      ...
      HQ_ConnectionSpec connectionSpec = connectionSpecSelector.configureSecureSocket(sslSocket);
      if (connectionSpec.supportsTlsExtensions()) {
        HQ_Platform.get().configureTlsExtensions(
            sslSocket, address.getUriHost(), address.getProtocols());
      }

      // Force handshake. This can throw!
      sslSocket.startHandshake();
      HQ_Handshake unverifiedHandshake = HQ_Handshake.get(sslSocket.getSession());

      // Verify that the socket's certificates are acceptable for the target host.
      if (!address.getHostnameVerifier().verify(address.getUriHost(), sslSocket.getSession())) {
        X509Certificate cert = (X509Certificate) unverifiedHandshake.peerCertificates().get(0);
        throw new SSLPeerUnverifiedException("Hostname " + address.getUriHost() + " not verified:"
            + "\n    certificate: " + HQ_CertificatePinner.pin(cert)
            + "\n    DN: " + cert.getSubjectDN().getName()
            + "\n    subjectAltNames: " + HQ_OkHostnameVerifier.allSubjectAltNames(cert));
      }
      ...
      String maybeProtocol = connectionSpec.supportsTlsExtensions()
          ? HQ_Platform.get().getSelectedProtocol(sslSocket)
          : null;
      protocol = maybeProtocol != null
          ? HQ_Protocol.get(maybeProtocol)
          : HQ_Protocol.HTTP_1_1;
      handshake = unverifiedHandshake;
      socket = sslSocket;
      success = true;
    } catch (AssertionError e) {
      if (HQ_Util.isAndroidGetsocknameError(e)) throw new IOException(e);
      throw e;
    } finally {
      if (sslSocket != null) {
        HQ_Platform.get().afterHandshake(sslSocket);
      }
      if (!success) {
   ...
   */
  private void createTunnel(int readTimeout, int writeTimeout, HQ_Request request) throws IOException {
    // Make an SSL Tunnel on the first message pair of each SSL + proxy connection.
    HQ_Request tunnelRequest = createTunnelRequest(request);
    HQ_HttpConnection tunnelConnection = new HQ_HttpConnection(pool, this, socket);
    tunnelConnection.setTimeouts(readTimeout, writeTimeout);
    HQ_HttpUrl url = tunnelRequest.httpUrl();
    String requestLine = "CONNECT " + url.host() + ":" + url.port() + " HTTP/1.1";
    while (true) {
      tunnelConnection.writeRequest(tunnelRequest.headers(), requestLine);
      tunnelConnection.flush();
      HQ_Response response = tunnelConnection.readResponse().request(tunnelRequest).build();
      ...
      HQ_Util.skipAll(body, Integer.MAX_VALUE, TimeUnit.MILLISECONDS);
      body.close();
      ...
      switch (response.code()) {
        ...
        case HTTP_PROXY_AUTH:
          tunnelRequest = HQ_OkHeaders.processAuthHeader(
   ...
   */
  private HQ_Request createTunnelRequest(HQ_Request request) throws IOException {
    HQ_HttpUrl tunnelUrl = new HQ_HttpUrl.Builder()
        .scheme("https")
        .host(request.httpUrl().host())
        .port(request.httpUrl().port())
        .build();
    HQ_Request.Builder result = new HQ_Request.Builder()
        .url(tunnelUrl)
        .header("Host", HQ_Util.hostHeader(tunnelUrl))
        .header("Proxy-HQ_Connection", "Keep-Alive"); // For HTTP/1.0 proxies like Squid.
  ...
  public HQ_Handshake getHandshake() {
    return handshake;
  }

  /** Returns the transport appropriate for this connection. */
  HQ_Transport newTransport(HQ_HttpEngine httpEngine) throws IOException {
    return (framedConnection != null)
        ? new HQ_FramedTransport(httpEngine, framedConnection)
        : new HQ_HttpTransport(httpEngine, httpConnection);
  }
   ...
   * HQ_Protocol#HTTP_1_1} if no protocol has been negotiated.
   */
   ...
   */
  void setProtocol(HQ_Protocol protocol) {

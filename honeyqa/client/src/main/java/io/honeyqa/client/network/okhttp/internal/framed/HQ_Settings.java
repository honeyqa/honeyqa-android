...
/**
 * HQ_Settings describe characteristics of the sending peer, which are used by the receiving peer.
 * HQ_Settings are {@link HQ_FramedConnection connection} scoped.
 */
public final class HQ_Settings {
  /**
  ...
  HQ_Settings set(int id, int idFlags, int value) {
    if (id >= values.length) {
  ...
  int flags(int id) {
    int result = 0;
    if (isPersisted(id)) result |= HQ_Settings.PERSISTED;
    if (persistValue(id)) result |= HQ_Settings.PERSIST_VALUE;
    return result;
  }

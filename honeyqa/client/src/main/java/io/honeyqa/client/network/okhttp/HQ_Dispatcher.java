...
import io.honeyqa.client.network.okhttp.HQ_Call.AsyncCall;
...
public final class HQ_Dispatcher {
  ...
  private final Deque<HQ_Call> executedCalls = new ArrayDeque<>();

  public HQ_Dispatcher(ExecutorService executorService) {
    this.executorService = executorService;
  }

  public HQ_Dispatcher() {
  }

  public synchronized ExecutorService getExecutorService() {
    if (executorService == null) {
      executorService = new ThreadPoolExecutor(0, Integer.MAX_VALUE, 60, TimeUnit.SECONDS,
          new SynchronousQueue<Runnable>(), HQ_Util.threadFactory("OkHttp HQ_Dispatcher", false));
    }
  ...
  public synchronized void cancel(Object tag) {
    ...
    for (AsyncCall call : runningCalls) {
      if (HQ_Util.equal(tag, call.tag())) {
        ...
        HQ_HttpEngine engine = call.get().engine;
        if (engine != null) engine.disconnect();
      }
    }

    for (HQ_Call call : executedCalls) {
      if (HQ_Util.equal(tag, call.tag())) {
        call.cancel();
      }
    }
  ...
  /** Used by {@code HQ_Call#execute} to signal it is in-flight. */
  synchronized void executed(HQ_Call call) {
    executedCalls.add(call);
  }

  /** Used by {@code HQ_Call#execute} to signal completion. */
  synchronized void finished(HQ_Call call) {
    if (!executedCalls.remove(call)) throw new AssertionError("HQ_Call wasn't in-flight!");
  }

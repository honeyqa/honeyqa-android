...
import io.honeyqa.client.network.okio.ForwardingTimeout;
...
import static io.honeyqa.client.network.okhttp.internal.HQ_Util.checkOffsetAndCount;
import static io.honeyqa.client.network.okhttp.internal.http.HQ_StatusLine.HTTP_CONTINUE;
import static io.honeyqa.client.network.okhttp.internal.http.HQ_Transport.DISCARD_STREAM_TIMEOUT_MILLIS;
 ...
 * closing that source.
 */
public final class HQ_HttpConnection {
  ...
  private final HQ_Connection connection;
  ...
  public HQ_HttpConnection(HQ_ConnectionPool pool, HQ_Connection connection, Socket socket)
      throws IOException {
    this.pool = pool;
  ...
  public void poolOnIdle() {
    ...
    if (state == STATE_IDLE) {
      ...
      HQ_Internal.instance.recycle(pool, connection);
    }
  }
  ...
  public void closeIfOwnedBy(Object owner) throws IOException {
    HQ_Internal.instance.closeIfOwnedBy(connection, owner);
  }
  ...
  public void writeRequest(HQ_Headers headers, String requestLine) throws IOException {
  ...
  public HQ_Response.Builder readResponse() throws IOException {
    ...
    try {
      while (true) {
        ...
        HQ_Response.Builder responseBuilder = new HQ_Response.Builder()
        ...
        headersBuilder.add(HQ_OkHeaders.SELECTED_PROTOCOL, statusLine.protocol.toString());
      ...
      IOException exception = new IOException("unexpected end of stream on " + connection
          + " (recycle count=" + HQ_Internal.instance.recycleCount(connection) + ")");
  ...
  public void readHeaders(HQ_Headers.Builder builder) throws IOException {
    // parse the result headers until the first blank line
    for (String line; (line = source.readUtf8LineStrict()).length() != 0; ) {
      HQ_Internal.instance.addLenient(builder, line);
    }
  }
  ...
  public void writeRequestBody(HQ_RetryableSink requestBody) throws IOException {
    if (state != STATE_OPEN_REQUEST_BODY) throw new IllegalStateException("state: " + state);
  ...
  public Source newChunkedSource(HQ_HttpEngine httpEngine) throws IOException {
    if (state != STATE_OPEN_RESPONSE_BODY) throw new IllegalStateException("state: " + state);
    state = STATE_READING_RESPONSE_BODY;
  ...
  private abstract class AbstractSource implements Source {
    ...
    protected final void unexpectedEndOfInput() {
      HQ_Util.closeQuietly(connection.getSocket());
      state = STATE_CLOSED;
    }
  }
  ...
  private class FixedLengthSource extends AbstractSource {
    ...
    @Override public void close() throws IOException {
      ...
      if (bytesRemaining != 0
          && !HQ_Util.discard(this, DISCARD_STREAM_TIMEOUT_MILLIS, MILLISECONDS)) {
  ...
  private class ChunkedSource extends AbstractSource {
    ...
    ChunkedSource(HQ_HttpEngine httpEngine) throws IOException {
    ...
    private void readChunkSize() throws IOException {
      ...
      if (bytesRemainingInChunk == 0L) {
        hasMoreChunks = false;
        HQ_Headers.Builder trailersBuilder = new HQ_Headers.Builder();
    ...
    @Override public void close() throws IOException {
      if (closed) return;
      if (hasMoreChunks && !HQ_Util.discard(this, DISCARD_STREAM_TIMEOUT_MILLIS, MILLISECONDS)) {
        unexpectedEndOfInput();
      }
      closed = true;
    }

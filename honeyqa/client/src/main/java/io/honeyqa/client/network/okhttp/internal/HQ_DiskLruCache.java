 ...
 */
public final class HQ_DiskLruCache implements Closeable {
  static final String JOURNAL_FILE = "journal";
  static final String JOURNAL_FILE_TEMP = "journal.tmp";
  static final String JOURNAL_FILE_BACKUP = "journal.bkp";
  static final String MAGIC = "libcore.io.HQ_DiskLruCache";
  ...
  private static final String READ = "READ";
     ...
     * constant string "libcore.io.HQ_DiskLruCache", the disk cache's version,
  ...
  private final HQ_FileSystem fileSystem;
  ...
  private final Runnable cleanupRunnable = new Runnable() {
    public void run() {
      synchronized (HQ_DiskLruCache.this) {
  ...
  HQ_DiskLruCache(HQ_FileSystem fileSystem, File directory, int appVersion, int valueCount, long maxSize,
                  Executor executor) {
  ...
  public synchronized void initialize() throws IOException {
    ...
    if (fileSystem.exists(journalFile)) {
      try {
        ...
        HQ_Platform.get().logW("HQ_DiskLruCache " + directory + " is corrupt: "
   ...
   */
  public static HQ_DiskLruCache create(HQ_FileSystem fileSystem, File directory, int appVersion,
    ...
    Executor executor = new ThreadPoolExecutor(0, 1, 60L, TimeUnit.SECONDS,
        new LinkedBlockingQueue<Runnable>(), HQ_Util.threadFactory("OkHttp HQ_DiskLruCache", true));

    return new HQ_DiskLruCache(fileSystem, directory, appVersion, valueCount, maxSize, executor);
  }
  ...
  private BufferedSink newJournalWriter() throws FileNotFoundException {
    ...
    Sink faultHidingSink = new HQ_FaultHidingSink(fileSink) {
      @Override protected void onException(IOException e) {
        assert (Thread.holdsLock(HQ_DiskLruCache.this));
  ...
  public synchronized Iterator<Snapshot> snapshots() throws IOException {
    ...
    return new Iterator<Snapshot>() {
      ...
      @Override public void remove() {
        ...
        try {
          HQ_DiskLruCache.this.remove(removeSnapshot.key);
  ...
  public final class Snapshot implements Closeable {
    ...
    public Editor edit() throws IOException {
      return HQ_DiskLruCache.this.edit(key, sequenceNumber);
    }
  ...
  public final class Editor {
    ...
    public Sink newSink(int index) throws IOException {
      synchronized (HQ_DiskLruCache.this) {
        ...
        return new HQ_FaultHidingSink(sink) {
          @Override protected void onException(IOException e) {
            synchronized (HQ_DiskLruCache.this) {
              hasErrors = true;
            }
  ...
  private final class Entry {
     ...
     */
    Snapshot snapshot() {
      if (!Thread.holdsLock(HQ_DiskLruCache.this)) throw new AssertionError();
      ...
      try {
        ...
        for (int i = 0; i < valueCount; i++) {
          if (sources[i] != null) {
            HQ_Util.closeQuietly(sources[i]);
          } else {
            break;
          }
        }

 ...
 */
public interface HQ_Authenticator {
  /**
   ...
   * <pre>   {@code
   *
   *    String credential = HQ_Credentials.basic(...)
   *    return response.request().newBuilder()
   *        .header("Authorization", credential)
   *        .build();
   * }</pre>
   */
  HQ_Request authenticate(Proxy proxy, HQ_Response response) throws IOException;
   ...
   * }</pre>
   */
  HQ_Request authenticateProxy(Proxy proxy, HQ_Response response) throws IOException;
}

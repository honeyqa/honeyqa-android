...
import io.honeyqa.client.network.okio.InflaterSource;
...
class HQ_NameValueBlockReader {
  ...
  public HQ_NameValueBlockReader(BufferedSource source) {
    ...
    Inflater inflater = new Inflater() {
      @Override public int inflate(byte[] buffer, int offset, int count)
        ...
        if (result == 0 && needsDictionary()) {
          setDictionary(HQ_Spdy3.DICTIONARY);
  ...
  public List<HQ_Header> readNameValueBlock(int length) throws IOException {
    ...
    List<HQ_Header> entries = new ArrayList<>(numberOfPairs);
    for (int i = 0; i < numberOfPairs; i++) {
      ...
      entries.add(new HQ_Header(name, values));
    }

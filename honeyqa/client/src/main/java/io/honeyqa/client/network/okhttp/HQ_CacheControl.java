...
import io.honeyqa.client.network.okhttp.internal.http.HQ_HeaderParser;
import java.util.concurrent.TimeUnit;

/**
 * A HQ_Cache-Control header with cache directives from a server or client. These
 ...
 * 2616, 14.9</a>.
 */
public final class HQ_CacheControl {
  /**
   * HQ_Cache control request directives that require network validation of
   * responses. Note that such requests may be assisted by the cache via
   * conditional GET requests.
   */
  public static final HQ_CacheControl FORCE_NETWORK = new Builder().noCache().build();

  /**
   * HQ_Cache control request directives that uses the cache only, even if the
   ...
   * Unsatisfiable HQ_Request}.
   */
  public static final HQ_CacheControl FORCE_CACHE = new Builder()
      .onlyIfCached()
  ...
  private HQ_CacheControl(boolean noCache, boolean noStore, int maxAgeSeconds, int sMaxAgeSeconds,
  ...
  private HQ_CacheControl(Builder builder) {
   ...
   * HQ_Cache-Control and Pragma headers if they are present.
   */
  public static HQ_CacheControl parse(HQ_Headers headers) {
    ...
    for (int i = 0, size = headers.size(); i < size; i++) {
      ...
      if (name.equalsIgnoreCase("HQ_Cache-Control")) {
        if (headerValue != null) {
      ...
      while (pos < value.length()) {
        ...
        if ("no-cache".equalsIgnoreCase(directive)) {
          ...
          maxAgeSeconds = HQ_HeaderParser.parseSeconds(parameter, -1);
        } else if ("s-maxage".equalsIgnoreCase(directive)) {
          sMaxAgeSeconds = HQ_HeaderParser.parseSeconds(parameter, -1);
          ...
          maxStaleSeconds = HQ_HeaderParser.parseSeconds(parameter, Integer.MAX_VALUE);
        } else if ("min-fresh".equalsIgnoreCase(directive)) {
          minFreshSeconds = HQ_HeaderParser.parseSeconds(parameter, -1);
    ...
    return new HQ_CacheControl(noCache, noStore, maxAgeSeconds, sMaxAgeSeconds, isPrivate, isPublic,
  ...
  /** Builds a {@code HQ_Cache-Control} request header. */
  public static final class Builder {
     ...
     * cached, a {@code 504 Unsatisfiable HQ_Request} response will be returned.
     */
    ...
    public HQ_CacheControl build() {
      return new HQ_CacheControl(this);
    }
  }
}

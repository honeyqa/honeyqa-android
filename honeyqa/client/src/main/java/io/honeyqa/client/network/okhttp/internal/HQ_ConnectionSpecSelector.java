 ...
 */
public final class HQ_ConnectionSpecSelector {

  private final List<HQ_ConnectionSpec> connectionSpecs;
  private int nextModeIndex;
  private boolean isFallbackPossible;
  private boolean isFallback;

  public HQ_ConnectionSpecSelector(List<HQ_ConnectionSpec> connectionSpecs) {
   ...
   * {@link HQ_ConnectionSpec}. Returns the chosen {@link HQ_ConnectionSpec}, never {@code null}.
   *
   * @throws IOException if the socket does not support any of the TLS modes available
   */
  public HQ_ConnectionSpec configureSecureSocket(SSLSocket sslSocket) throws IOException {
    HQ_ConnectionSpec tlsConfiguration = null;
    for (int i = nextModeIndex, size = connectionSpecs.size(); i < size; i++) {
      HQ_ConnectionSpec connectionSpec = connectionSpecs.get(i);
    ...
    HQ_Internal.instance.apply(tlsConfiguration, sslSocket, isFallback);

    return tlsConfiguration;
  }

  /**
   * Reports a failure to complete a connection. Determines the next {@link HQ_ConnectionSpec} to
   * try, if any.
   *
  ...
  public boolean connectionFailed(IOException e) {
    ...
    // HQ_ConnectionSpec: we assume it is unreachable.
  ...
  /**
   * Returns {@code true} if any later {@link HQ_ConnectionSpec} in the fallback strategy looks

 ...
 *
 * <p>HTTP requests that share the same {@code HQ_Address} may also share the same
 * {@link HQ_Connection}.
 */
public final class HQ_Address {
  final Proxy proxy;
  ...
  final HQ_CertificatePinner certificatePinner;
  final HQ_Authenticator authenticator;
  final List<HQ_Protocol> protocols;
  final List<HQ_ConnectionSpec> connectionSpecs;
  final ProxySelector proxySelector;

  public HQ_Address(String uriHost, int uriPort, SocketFactory socketFactory,
                    SSLSocketFactory sslSocketFactory, HostnameVerifier hostnameVerifier,
                    HQ_CertificatePinner certificatePinner, HQ_Authenticator authenticator, Proxy proxy,
                    List<HQ_Protocol> protocols, List<HQ_ConnectionSpec> connectionSpecs, ProxySelector proxySelector) {
   ...
   * non-null list that contains minimally {@link HQ_Protocol#HTTP_1_1}.
   */
  ...
  @Override public boolean equals(Object other) {
    if (other instanceof HQ_Address) {
      HQ_Address that = (HQ_Address) other;

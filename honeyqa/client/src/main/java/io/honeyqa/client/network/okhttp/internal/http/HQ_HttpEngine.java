...
import io.honeyqa.client.network.okhttp.HQ_Address;
import io.honeyqa.client.network.okhttp.HQ_CertificatePinner;
...
import io.honeyqa.client.network.okhttp.HQ_Interceptor;
...
import io.honeyqa.client.network.okhttp.internal.HQ_Version;
...
import io.honeyqa.client.network.okio.GzipSource;
...
import io.honeyqa.client.network.okio.Timeout;

import static io.honeyqa.client.network.okhttp.internal.HQ_Util.closeQuietly;
import static io.honeyqa.client.network.okhttp.internal.http.HQ_StatusLine.HTTP_CONTINUE;
 ...
 */
public final class HQ_HttpEngine {
    /**
    ...
    private static final HQ_ResponseBody EMPTY_BODY = new HQ_ResponseBody() {
        @Override
    ...
    final HQ_OkHttpClient client;

    private HQ_Connection connection;
    private HQ_Address address;
    private HQ_RouteSelector routeSelector;
    private HQ_Route route;
    private final HQ_Response priorResponse;

    private HQ_Transport transport;
     ...
     */
    private final HQ_Request userRequest;
     ...
     */
    private HQ_Request networkRequest;
     ...
     */
    private HQ_Response userResponse;
     ...
     */
    private HQ_CacheRequest storeRequest;
    private HQ_CacheStrategy cacheStrategy;
     ...
     */
    public HQ_HttpEngine(HQ_OkHttpClient client, HQ_Request request, boolean bufferRequestBody,
                         boolean callerWritesRequestBody, boolean forWebSocket, HQ_Connection connection,
                         HQ_RouteSelector routeSelector, HQ_RetryableSink requestBodyOut, HQ_Response priorResponse) {
        this.client = client;
        ...
        if (connection != null) {
            HQ_Internal.instance.setOwner(connection, this);
     ...
     *
     * @throws HQ_RequestException if there was a problem with request setup. Unrecoverable.
     * @throws HQ_RouteException   if the was a problem during connection via a specific route. Sometimes
     *                          recoverable. See {@link #recover(HQ_RouteException)}.
     * @throws IOException      if there was a problem while making a request. Sometimes recoverable. See
     *                          {@link #recover(IOException)}.
     */
    public void sendRequest() throws HQ_RequestException, HQ_RouteException, IOException {
        if (cacheStrategy != null) return; // Already sent.
        if (transport != null) throw new IllegalStateException();

        HQ_Request request = networkRequest(userRequest);

        HQ_InternalCache responseCache = HQ_Internal.instance.internalCache(client);
        HQ_Response cacheCandidate = responseCache != null
                ? responseCache.get(request)
                : null;

        long now = System.currentTimeMillis();
        cacheStrategy = new HQ_CacheStrategy.Factory(now, request, cacheCandidate).get();
        ...
        if (networkRequest != null) {
            ...
            transport = HQ_Internal.instance.newTransport(connection, this);
            ...
            if (callerWritesRequestBody && permitsRequestBody(networkRequest) && requestBodyOut == null) {
                long contentLength = HQ_OkHeaders.contentLength(request);
                if (bufferRequestBody) {
                    ...
                    if (contentLength != -1) {
                        ...
                        requestBodyOut = new HQ_RetryableSink((int) contentLength);
                    } else {
                        // Buffer a request body of an unknown length. Don't write request
                        // headers until the entire body is ready; otherwise we can't set the
                        // Content-Length header correctly.
                        requestBodyOut = new HQ_RetryableSink();
                    }
                } else {
                    transport.writeRequestHeaders(networkRequest);
            ...
            if (connection != null) {
                HQ_Internal.instance.recycle(client.getConnectionPool(), connection);
                connection = null;
            }
            ...
            if (cacheResponse != null) {
                ...
                this.userResponse = new HQ_Response.Builder()
                        .request(userRequest)
                        .priorResponse(stripBody(priorResponse))
                        .protocol(HQ_Protocol.HTTP_1_1)
                        .code(504)
                        .message("Unsatisfiable HQ_Request (only-if-cached)")
                        .body(EMPTY_BODY)
                        .build();
            }
    ...
    private static HQ_Response stripBody(HQ_Response response) {
     ...
     */
    private void connect() throws HQ_RequestException, HQ_RouteException {
        if (connection != null) throw new IllegalStateException();

        if (routeSelector == null) {
            address = createAddress(client, networkRequest);
            try {
                routeSelector = HQ_RouteSelector.get(address, networkRequest, client);
            } catch (IOException e) {
                throw new HQ_RequestException(e);
            }
        }

        connection = createNextConnection();
        HQ_Internal.instance.connectAndSetOwner(client, connection, this, networkRequest);
        route = connection.getRoute();
    }

    private HQ_Connection createNextConnection() throws HQ_RouteException {
        HQ_ConnectionPool pool = client.getConnectionPool();

        // Always prefer pooled connections over new connections.
        for (HQ_Connection pooled; (pooled = pool.get(address)) != null; ) {
            if (networkRequest.method().equals("GET") || HQ_Internal.instance.isReadable(pooled)) {
                return pooled;
            }
        ...
        try {
            HQ_Route route = routeSelector.next();
            return new HQ_Connection(pool, route);
        } catch (IOException e) {
            throw new HQ_RouteException(e);
        }
    }
    ...
    boolean permitsRequestBody(HQ_Request request) {
        return HQ_HttpMethod.permitsRequestBody(request.method());
    }
    ...
    public HQ_Response getResponse() {
        if (userResponse == null) throw new IllegalStateException();
        return userResponse;
    }

    public HQ_Connection getConnection() {
        return connection;
    }
     ...
     */
    public HQ_HttpEngine recover(HQ_RouteException e) {
        ...
        HQ_Connection connection = close();

        // For failure recovery, use the same route selector with a new connection.
        return new HQ_HttpEngine(client, userRequest, bufferRequestBody, callerWritesRequestBody,
                forWebSocket, connection, routeSelector, (HQ_RetryableSink) requestBodyOut, priorResponse);
    }

    private boolean isRecoverable(HQ_RouteException e) {
     ...
     * body is buffered.
     */
    public HQ_HttpEngine recover(IOException e, Sink requestBodyOut) {
        if (routeSelector != null && connection != null) {
            connectFailed(routeSelector, e);
        }

        boolean canRetryRequestBody = requestBodyOut == null || requestBodyOut instanceof HQ_RetryableSink;
        if (routeSelector == null && connection == null // No connection.
                || routeSelector != null && !routeSelector.hasNext() // No more routes to attempt.
                || !isRecoverable(e)
                || !canRetryRequestBody) {
    ...
    private void connectFailed(HQ_RouteSelector routeSelector, IOException e) {
        // If this is a recycled connection, don't count its failure against the route.
        if (HQ_Internal.instance.recycleCount(connection) > 0) return;
        HQ_Route failedRoute = connection.getRoute();
        routeSelector.connectFailed(failedRoute, e);
    }

    public HQ_HttpEngine recover(IOException e) {
    ...
    private void maybeCache() throws IOException {
        ...
        if (!HQ_CacheStrategy.isCacheable(userResponse, networkRequest)) {
            if (HQ_HttpMethod.invalidatesCache(networkRequest.method())) {
                try {
    ...
    public void disconnect() {
        try {
            if (transport != null) {
                ...
                HQ_Connection connection = this.connection;
                if (connection != null) {
                    HQ_Internal.instance.closeIfOwnedBy(connection, this);
                }
            }
        } catch (IOException ignored) {
        }
    ...
    public HQ_Connection close() {
        ...
        if (connection != null && !HQ_Internal.instance.clearOwner(connection)) {
            connection = null;
        }

        HQ_Connection result = connection;
        connection = null;
        return result;
    }
     ...
     * <p/>
     * <p>This method should only be used for non-empty response bodies. HQ_Response codes like "304 Not
     ...
     */
    private HQ_Response unzip(final HQ_Response response) throws IOException {
        ...
        HQ_Headers strippedHeaders = response.headers().newBuilder()
                .removeAll("Content-Encoding")
                .removeAll("Content-Length")
                .build();
        return response.newBuilder()
                .headers(strippedHeaders)
                .body(new HQ_RealResponseBody(strippedHeaders, Okio.buffer(responseBody)))
                .build();
    }
     ...
     */
    public static boolean hasBody(HQ_Response response) {
        ...
        // honor the headers.
        if (HQ_OkHeaders.contentLength(response) != -1
     ...
     */
    private HQ_Request networkRequest(HQ_Request request) throws IOException {
        HQ_Request.Builder result = request.newBuilder();

        if (request.header("Host") == null) {
            result.header("Host", HQ_Util.hostHeader(request.httpUrl()));
        }

        if ((connection == null || connection.getProtocol() != HQ_Protocol.HTTP_1_0)
                && request.header("HQ_Connection") == null) {
            result.header("HQ_Connection", "Keep-Alive");
        }
        ...
        if (cookieHandler != null) {
            ...
            Map<String, List<String>> headers = HQ_OkHeaders.toMultimap(result.build().headers(), null);

            Map<String, List<String>> cookies = cookieHandler.get(request.uri(), headers);

            // Add any new cookies to the request.
            HQ_OkHeaders.addCookies(result, cookies);
        }

        if (request.header("User-Agent") == null) {
            result.header("User-Agent", HQ_Version.userAgent());
        }
    ...
    public void readResponse() throws IOException {
        ...
        if (forWebSocket) {
            ...
            if (sentRequestMillis == -1) {
                if (HQ_OkHeaders.contentLength(networkRequest) == -1
                        && requestBodyOut instanceof HQ_RetryableSink) {
                    long contentLength = ((HQ_RetryableSink) requestBodyOut).contentLength();
            ...
            if (requestBodyOut != null) {
                ...
                if (requestBodyOut instanceof HQ_RetryableSink) {
                    transport.writeRequestBody((HQ_RetryableSink) requestBodyOut);
                }
            }
    ...
    class NetworkInterceptorChain implements HQ_Interceptor.Chain {
        private final int index;
        private final HQ_Request request;
        private int calls;

        NetworkInterceptorChain(int index, HQ_Request request) {
            this.index = index;
            this.request = request;
        }

        @Override
        public HQ_Connection connection() {
            return connection;
        }
        ...
        @Override
        public HQ_Response proceed(HQ_Request request) throws IOException {
            calls++;

            if (index > 0) {
                HQ_Interceptor caller = client.networkInterceptors().get(index - 1);
                HQ_Address address = connection().getRoute().getAddress();
            ...
            if (index < client.networkInterceptors().size()) {
                ...
                HQ_Interceptor interceptor = client.networkInterceptors().get(index);
                HQ_Response interceptedResponse = interceptor.intercept(chain);
            ...
            HQ_Response response = readNetworkResponse();
    ...
    private HQ_Response readNetworkResponse() throws IOException {
        transport.finishRequest();

        HQ_Response networkResponse = transport.readResponseHeaders()
                .request(networkRequest)
                .handshake(connection.getHandshake())
                .header(HQ_OkHeaders.SENT_MILLIS, Long.toString(sentRequestMillis))
                .header(HQ_OkHeaders.RECEIVED_MILLIS, Long.toString(System.currentTimeMillis()))
                .build();
        ...
        HQ_Internal.instance.setProtocol(connection, networkResponse.protocol());
     ...
     */
    private HQ_Response cacheWritingResponse(final HQ_CacheRequest cacheRequest, HQ_Response response)
            throws IOException {
        ...
        Source cacheWritingSource = new Source() {
            ...
            public void close() throws IOException {
                if (!cacheRequestClosed
                        && !HQ_Util.discard(this, HQ_Transport.DISCARD_STREAM_TIMEOUT_MILLIS, MILLISECONDS)) {
                    cacheRequestClosed = true;
                    cacheRequest.abort();
                }
                source.close();
        ...
        return response.newBuilder()
                .body(new HQ_RealResponseBody(response.headers(), Okio.buffer(cacheWritingSource)))
                .build();
    }
     ...
     */
    private static boolean validate(HQ_Response cached, HQ_Response network) {
     ...
     * 13.5.3.
     */
    private static HQ_Headers combine(HQ_Headers cachedHeaders, HQ_Headers networkHeaders) throws IOException {
        HQ_Headers.Builder result = new HQ_Headers.Builder();
        ...
        for (int i = 0, size = cachedHeaders.size(); i < size; i++) {
            ...
            if (!HQ_OkHeaders.isEndToEnd(fieldName) || networkHeaders.get(fieldName) == null) {
        ...
        for (int i = 0, size = networkHeaders.size(); i < size; i++) {
            ...
            if (HQ_OkHeaders.isEndToEnd(fieldName)) {
    ...
    public void receiveHeaders(HQ_Headers headers) throws IOException {
        CookieHandler cookieHandler = client.getCookieHandler();
        if (cookieHandler != null) {
            cookieHandler.put(userRequest.uri(), HQ_OkHeaders.toMultimap(headers, null));
        }
    }
     ...
     */
    public HQ_Request followUpRequest() throws IOException {
        if (userResponse == null) throw new IllegalStateException();
        ...
        switch (responseCode) {
            ...
            case HTTP_UNAUTHORIZED:
                return HQ_OkHeaders.processAuthHeader(client.getAuthenticator(), userResponse, selectedProxy);
            ...
            case HTTP_SEE_OTHER:
                ...
                HQ_HttpUrl url = userRequest.httpUrl().resolve(location);
                ...
                HQ_Request.Builder requestBuilder = userRequest.newBuilder();
                if (HQ_HttpMethod.permitsRequestBody(userRequest.method())) {
     ...
     */
    public boolean sameConnection(HQ_HttpUrl followUp) {
        HQ_HttpUrl url = userRequest.httpUrl();
        return url.host().equals(followUp.host())
                && url.port() == followUp.port()
                && url.scheme().equals(followUp.scheme());
    }

    private static HQ_Address createAddress(HQ_OkHttpClient client, HQ_Request request) {
        SSLSocketFactory sslSocketFactory = null;
        HostnameVerifier hostnameVerifier = null;
        HQ_CertificatePinner certificatePinner = null;
        if (request.isHttps()) {
        ...
        return new HQ_Address(request.httpUrl().host(), request.httpUrl().port(),

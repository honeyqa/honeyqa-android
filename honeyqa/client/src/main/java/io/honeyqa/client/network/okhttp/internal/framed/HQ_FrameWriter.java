...
public interface HQ_FrameWriter extends Closeable {
  /** HTTP/2 only. */
  void connectionPreface() throws IOException;
  /** Informs the peer that we've applied its latest settings. */
  void ackSettings(HQ_Settings peerSettings) throws IOException;
   ...
   */
  void pushPromise(int streamId, int promisedStreamId, List<HQ_Header> requestHeaders)
      throws IOException;
  ...
  void synReply(boolean outFinished, int streamId, List<HQ_Header> headerBlock)
      throws IOException;
  void headers(int streamId, List<HQ_Header> headerBlock) throws IOException;
  void rstStream(int streamId, HQ_ErrorCode errorCode) throws IOException;
  ...
  void settings(HQ_Settings okHttpSettings) throws IOException;
   ...
   */
  void goAway(int lastGoodStreamId, HQ_ErrorCode errorCode, byte[] debugData) throws IOException;

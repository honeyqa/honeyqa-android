...
/**
 * {@link HQ_Protocol#HTTP_2 HTTP/2} only.
 ...
 */
public interface HQ_PushObserver {
  /**
   ...
   */
  boolean onRequest(int streamId, List<HQ_Header> requestHeaders);
   ...
   */
  boolean onHeaders(int streamId, List<HQ_Header> responseHeaders, boolean last);
  ...
  void onReset(int streamId, HQ_ErrorCode errorCode);

  HQ_PushObserver CANCEL = new HQ_PushObserver() {

    @Override public boolean onRequest(int streamId, List<HQ_Header> requestHeaders) {
      return true;
    }

    @Override public boolean onHeaders(int streamId, List<HQ_Header> responseHeaders, boolean last) {
      return true;
    }
    ...
    @Override public void onReset(int streamId, HQ_ErrorCode errorCode) {
    }
  };
}

...
import io.honeyqa.client.network.okio.DeflaterSink;
 ...
 */
public final class HQ_Spdy3 implements HQ_Variant {

  @Override public HQ_Protocol getProtocol() {
    return HQ_Protocol.SPDY_3;
  }
  ...
  static {
    try {
      DICTIONARY = ("\u0000\u0000\u0000\u0007options\u0000\u0000\u0000\u0004hea"
          ...
          + "403 Forbidden404 Not Found500 HQ_Internal Server Error501 Not Implemented503 Service Un"
          ...
          + "sdchcharset=utf-8charset=iso-8859-1,utf-,*,enq=0.").getBytes(HQ_Util.UTF_8.name());
    } catch (UnsupportedEncodingException e) {
      throw new AssertionError();
    }
  }

  @Override public HQ_FrameReader newReader(BufferedSource source, boolean client) {
    return new Reader(source, client);
  }

  @Override public HQ_FrameWriter newWriter(BufferedSink sink, boolean client) {
    return new Writer(sink, client);
  }

  /** Read spdy/3 frames. */
  static final class Reader implements HQ_FrameReader {
    private final BufferedSource source;
    private final boolean client;
    private final HQ_NameValueBlockReader headerBlockReader;

    Reader(BufferedSource source, boolean client) {
      this.source = source;
      this.headerBlockReader = new HQ_NameValueBlockReader(this.source);
      this.client = client;
    }
    ...
    private void readSynStream(Handler handler, int flags, int length) throws IOException {
      ...
      List<HQ_Header> headerBlock = headerBlockReader.readNameValueBlock(length - 10);
      ...
      handler.headers(outFinished, inFinished, streamId, associatedStreamId, headerBlock,
          HQ_HeadersMode.SPDY_SYN_STREAM);
    }

    private void readSynReply(Handler handler, int flags, int length) throws IOException {
      int w1 = source.readInt();
      int streamId = w1 & 0x7fffffff;
      List<HQ_Header> headerBlock = headerBlockReader.readNameValueBlock(length - 4);
      boolean inFinished = (flags & FLAG_FIN) != 0;
      handler.headers(false, inFinished, streamId, -1, headerBlock, HQ_HeadersMode.SPDY_REPLY);
    }
    ...
    private void readRstStream(Handler handler, int flags, int length) throws IOException {
      ...
      HQ_ErrorCode errorCode = HQ_ErrorCode.fromSpdy3Rst(errorCodeInt);
      if (errorCode == null) {
    ...
    private void readHeaders(Handler handler, int flags, int length) throws IOException {
      ...
      int streamId = w1 & 0x7fffffff;
      List<HQ_Header> headerBlock = headerBlockReader.readNameValueBlock(length - 4);
      handler.headers(false, false, streamId, -1, headerBlock, HQ_HeadersMode.SPDY_HEADERS);
    }
    ...
    private void readGoAway(Handler handler, int flags, int length) throws IOException {
      ...
      int errorCodeInt = source.readInt();
      HQ_ErrorCode errorCode = HQ_ErrorCode.fromSpdyGoAway(errorCodeInt);
      if (errorCode == null) {
    ...
    private void readSettings(Handler handler, int flags, int length) throws IOException {
      ...
      HQ_Settings settings = new HQ_Settings();
      ...
      boolean clearPrevious = (flags & HQ_Settings.FLAG_CLEAR_PREVIOUSLY_PERSISTED_SETTINGS) != 0;
  ...
  /** Write spdy/3 frames. */
  static final class Writer implements HQ_FrameWriter {
    ...
    @Override public void ackSettings(HQ_Settings peerSettings) {
    ...
    @Override public synchronized void synStream(boolean outFinished, boolean inFinished,
        int streamId, int associatedStreamId, List<HQ_Header> headerBlock)
        throws IOException {
      if (closed) throw new IOException("closed");
    ...
    @Override public synchronized void synReply(boolean outFinished, int streamId,
        List<HQ_Header> headerBlock) throws IOException {
      if (closed) throw new IOException("closed");
      writeNameValueBlockToBuffer(headerBlock);
    ...
    @Override public synchronized void headers(int streamId, List<HQ_Header> headerBlock)
        throws IOException {
      if (closed) throw new IOException("closed");
      writeNameValueBlockToBuffer(headerBlock);
      int flags = 0;
    ...
    @Override public synchronized void rstStream(int streamId, HQ_ErrorCode errorCode)
        throws IOException {
      if (closed) throw new IOException("closed");
    ...
    private void writeNameValueBlockToBuffer(List<HQ_Header> headerBlock) throws IOException {
    ...
    @Override public synchronized void settings(HQ_Settings settings) throws IOException {
      if (closed) throw new IOException("closed");
      ...
      sink.writeInt((flags & 0xff) << 24 | length & 0xffffff);
      sink.writeInt(size);
      for (int i = 0; i <= HQ_Settings.COUNT; i++) {
    ...
    @Override public synchronized void goAway(int lastGoodStreamId, HQ_ErrorCode errorCode,
    ...
    @Override public synchronized void close() throws IOException {
      closed = true;
      HQ_Util.closeAll(sink, headerBlockOut);
    }
  }
}
